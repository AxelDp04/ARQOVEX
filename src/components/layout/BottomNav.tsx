...
export default function BottomNav() {
    ...
    return (
        <div className="md:hidden fixed bottom-6 left-1/2 -translate-x-1/2 w-[95%] max-w-[390px] z-50 pointer-events-none">
            <div className="grid grid-cols-5 bg-[#050810]/90 backdrop-blur-3xl border border-white/10 rounded-[1.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.6)] px-1 py-2 h-16 pointer-events-auto items-center">
                {navItems.map((item, idx) => {
                    ...
                    const innerContent = (
                        <>
                            ...
                            <span className={`text-[8px] font-bold uppercase tracking-tighter truncate w-full text-center px-0.5 transition-colors ${
                    ...
                    if (item.isExternal) {
                        return (
                            <a
                                ...
                                className="flex flex-col items-center justify-center gap-1 group active:scale-95 transition-all overflow-hidden"
                            >
                                <div className="p-1.5 rounded-xl bg-green-500/10 text-green-500 group-hover:bg-green-500/20 transition-colors">
                                    <Icon className="w-4.5 h-4.5" />
                                </div>
                                <span className="text-[8px] font-bold uppercase tracking-tighter truncate w-full text-center px-0.5 text-gray-400">{item.label}</span>
                            </a>
                        );
                    }
