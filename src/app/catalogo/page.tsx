    ...
    useEffect(() => {
        const fetchPlanos = async () => {
            ...
            try {
                ...
                if (error) {
                    console.error("Supabase Error [Catalogo]:", error.message, error.details);
                    setPlanos([]);
                } else if (data) {
                    const filteredData = (data as Plano[]).filter((p: Plano) => {
                ...
                console.error("Fetch Exception [Catalogo]:", err);
            }
            setLoading(false);
        };
        fetchPlanos();
    }, [isSeccionInmobiliaria, supabase]);
