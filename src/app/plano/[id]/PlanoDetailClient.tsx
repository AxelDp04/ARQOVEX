...
interface PlanoDetailClientProps {
    initialPlano: Plano;
    initialGaleriaUrls: string[];
    initialAvgRating: number | null;
    initialNumResenas: number;
}

export default function PlanoDetailClient({ 
    initialPlano, 
    initialGaleriaUrls, 
    initialAvgRating, 
    initialNumResenas 
}: PlanoDetailClientProps) {
    const params = useParams();
    const router = useRouter();
    const [plano, setPlano] = useState<Plano>(initialPlano);
    const [galeriaUrls, setGaleriaUrls] = useState<string[]>(initialGaleriaUrls);
    const [activeImgIndex, setActiveImgIndex] = useState(0);
    const [showWhatsAppModal, setShowWhatsAppModal] = useState(false);

    // Initial loading is practically 0 since we have SSR data
    ...
    useEffect(() => {
        const fetchUserContextAndStats = async () => {
            // Get user
            const { data: { user } } = await supabase.auth.getUser();
            setUserContext(user);

            // Check user states if logged in against the initialPlano
            if (user && initialPlano) {
                try {
                    const planoId = initialPlano.id;

                    const { data: favs } = await supabase.from("favoritos").select("id").eq("user_id", user.id).eq("plano_id", planoId);
                    if (favs && favs.length > 0) setIsFavorito(true);

                    if (initialPlano.tipo_propiedad === "Plano Arquitectónico") {
                    ...
                    console.error("User context check failed:", err);
                }
            }
            setLoading(false);
        };
            ...
            const channel = supabase
                ...
                .on(
                    ...
                    (payload) => {
                        ...
                        setPlano(prev => ({ ...prev, total_vistas: newVistas }));
                    }
                )
                .subscribe();
    ...
    // Eliminar JSX the loading fallback ya que cargará estáticamente
